class ResourcesController < ApplicationController
  ...
  def list_1
    university_list = {list: JSON.parse($redis_cable.get('libs:universities')),
                        hot_list: %w(南京大学 东南大学 南京理工大学)}
    city_list = {list: JSON.parse($redis_cable.get('libs:cities')),
                  hot_list: %w(南京 北京 上海 深圳)}
    major_list = {list: JSON.parse($redis_cable.get('libs:majors')),
                  hot_list: %w(计算机 物理 化工 销售)}
    job_list = {list: JSON.parse($redis_cable.get('libs:positions')),
                hot_list: JSON.parse($redis_cable.get('libs:hot_positions'))}
    ...
    render json: {code: 0, data: {
        ...
        job_list: job_list,
        industries_tag_list: industries_tag_list,
  ...
  def list_2
    industry_list = {list: JSON.parse($redis_cable.get('libs:industries')),
                     hot_list: JSON.parse($redis_cable.get('libs:industries'))}
    ...
    skill_list = {
        list: JSON.parse($redis_cable.get('libs:skills')),
        hot_list: JSON.parse($redis_cable.get('libs:skills'))
    }

    experience_title_list = {
        list: JSON.parse($redis_cable.get('libs:experiences')),
        hot_list: JSON.parse($redis_cable.get('libs:experiences'))
    }
    ...
    render json: {code: 0, data:
        {
            ...
            experience_title_list: experience_title_list}
    }
