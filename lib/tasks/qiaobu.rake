namespace :qiaobu do
  ...
  desc 'extract_all_from_qiaobu'
  task extract_all_from_qiaobu: [:extract_benefits_skills_categories,:extract_honors, :extract_interest_tags,:extract_industry, :extract_industry_experiences] do
    puts "extract end"
  end
  ...
  task extract_benefits_skills_categories: :environment do
    ...
    puts "[乔布简历] benefits"
    puts benefits
    benefits.each { |b| ResumeDicBenefit.find_or_create_by name: b }
    skills = formatted['result'].map { |t| t['job']['skill']&.split /[\,\，\、\、]/ }.flatten.uniq
    puts "[乔布简历] skills"
    puts skills

    skills.each { |s| ResumeDicSkill.find_or_create_by name: s }
    categories = formatted['result'].map { |t| t['job']['categories'] }.flatten.uniq
    puts "[乔布简历] categories"
    puts categories
    categories.each { |c| ResumeDicCategory.find_or_create_by name: c }
  end
  ...
  task extract_honors: :environment do
    ...
    formatted['career_honor_tips'].each do |item|
      name = item['name']
      pp name
      ResumeDicHonorTip.create! name: name, tips: item['tips']
    end
  end
    ...
    %w(
     ...
     http://cv.qiaobutang.com/rest/app/connection/interested/tags.json?at=1499523495859&intentName=%E5%B8%82%E5%9C%BA%E8%90%A5%E9%94%80&resolution=4x&sig=46059cf3203bd93b68135c5033831e75&uid=57d180030cf2123d500195a9&v=13
      ...
      formatted['tags'].each do |json|
        pp json
        ResumeDicInterestTag.create! intent: intent, name: json['name']
      end
    end
  end
  ...
  task extract_industry: :environment do
    ...
    formatted['tags'].each do |json|
      pp json
      ResumeDicIndustry.create! q_uuid: json['id'], name: json['name']
    end
  end
  ...
  task extract_industry_experiences: :environment do
    %w(
      ...
      pp url
      q_uuid = url.split(/\&/)[1].split(/\=/)[1]
      uri = URI url
      agent = Mechanize.new
      page = agent.get uri
      industry = ResumeDicIndustry.find_by q_uuid: q_uuid
      puts 'industry: '
      puts industry.name
      JSON(page.body)['modelessays'].each do |m|
        e = ResumeDicExperience.find_by title: m['title']
        if e.nil?
          pp m['title']
